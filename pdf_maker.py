...
class PDF(FPDF):
    ...
    def __init__(self, rows, title, self_info_id, product_data_id):
        super().__init__()
        self.rows = rows
        self.title = title
    ...
    def my_company(self):
        ...
        company_adress = "Schulstraße 6"
        company_zip = "5620 Schwarzach"
        border_on= False
        widht_size = 5
        height_size = 100
        self.set_font("helvetica", size=10)
        self.cell(height_size, widht_size, txt=company_name, border=border_on)
        self.cell(height_size, widht_size, txt="Rechnunsnummer:", border=border_on, align="L", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.cell(height_size, widht_size, txt=company_adress, border=border_on)
        self.cell(height_size, widht_size, txt="Rechnungsdatum:", border=border_on, align="L", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.cell(height_size, widht_size, txt=company_zip, border=border_on)
        ...
        for i in range(self.rows):
            ...
            #Produktbezeichnung
            self.set_font("helvetica", "B", size=10)
            self.set_xy(40, row_1)
            self.cell(80, 8, txt=data[0][1], border=border_on)
            #Menge
            self.set_font("helvetica", size=10)
            self.set_xy(90, row_1)
            self.cell(20, 8, txt=str(int_to_comma(Anzahl)), border=border_on)
            #Einheit
            ...
            self.cell(30, 8, txt=str(int_to_comma(data[0][4]*Anzahl)) ,border=border_on)
            summe = data[0][4]*Anzahl+summe
            ust_summe = ((data[0][4]*Anzahl)/100)*20+ust_summe
            #Notiz
            self.set_xy(40, index_note_rows[i])
